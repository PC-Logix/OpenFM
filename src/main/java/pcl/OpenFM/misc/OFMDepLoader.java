...
public class OFMDepLoader implements IFMLLoadingPlugin, IFMLCallHook {
    ...
    public static class DepLoadInst {
        ...
        private LaunchClassLoader loader = (LaunchClassLoader) OFMDepLoader.class.getClassLoader();
        ...
        private void load(Dependency dep) {
            ...
            if (dep.existing == null && OFMDepLoader.class.getResource("/" + dep.testClass.replace('.', '/') + ".class") == null)
                install(dep);
        }
        ...
        private void scanDepInfo(File file) {
            try {
                ...
                ZipEntry e = zip.getEntry("ofmdeps.info");
                if (e != null)
                    loadJSon(zip.getInputStream(e));

                zip.close();
            } catch (Exception e) {
                logger.error("Failed to load ofmdeps.info from " + file.getName() + " as JSON", e);
            }
        }
        ...
        private void loadJson(JsonObject node) throws IOException {
            boolean obfuscated = ((LaunchClassLoader) OFMDepLoader.class.getClassLoader())
